...
extern "C" {
    ...
    Java_org_example_project_LlamaJni_generateNextToken(JNIEnv* env, jobject /*obj*/, jlong ctxPointer, jstring promptJ) {
        ...
        Java_org_example_project_LlamaJni_generateNextTokenStream(JNIEnv* env, jobject /*obj*/, jlong ctxPointer, jstring promptJ, jobject callback ) {

            jclass callbackCls = env->GetObjectClass(callback);
            jmethodID onToken = env->GetMethodID(callbackCls, "invoke", "(Ljava/lang/Object;)Ljava/lang/Object;");
            ...
            auto generate = [&](const std::string & prompt) {
                ...
                while (true) {
                    ...
                    env->CallObjectMethod(callback, onToken, jpiece);
            ...
            int new_len = llama_chat_apply_template(tmpl, messages.data(), messages.size(), true, formatted.data(), formatted.size());
            if (new_len > (int)formatted.size()) formatted.resize(new_len);
            std::string user_prompt(formatted.begin() + prev_len, formatted.begin() + new_len);
